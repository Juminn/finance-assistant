...
- **CI가 통과하면 머지까지 따로 묻지 않고 진행한다.** 브랜치 → PR → CI → 머지가
  한 흐름이고, 중간에 확인을 받으려고 멈추지 않는다.
  머지 방식은 `--rebase`(main 히스토리는 선형), 머지 후 원격 브랜치는 삭제한다.
  CI가 실패하면 머지하지 말고 원인을 고쳐 다시 올린다.
- main이 앞서가 충돌하면 `origin/main` 위로 리베이스한다. 프롬프트·골든셋처럼
  양쪽이 같은 파일을 고친 경우 한쪽을 버리지 말고 둘 다 살린 뒤 eval을 다시 돌린다.
