...
# 정책 예·적금이 공시 상품과 한 카테고리에 섞이면서 검색 풀이 커졌다 — 5건이면
# 전국 단위 정책 적금이 지자체 통장·시중 적금에 밀려 잘린다 (실측 2026-08:
# "청년 적금" 질의에서 청년미래적금 9위). 무관한 꼬리는 유사도 바닥이 걸러낸다.
_TOP_K = 10
...
def search_products_by_condition(query: str, category: str = "") -> str:
    ...
    정부·지자체의 각종 융자 지원 등 빌리는 성격 전부), 정책 적금·통장
    (청년미래적금·청년내일저축계좌 등 — 적금 카테고리에 함께 색인),
    정책지원(이자·보증료 지원, 보조금처럼 상품이 아닌 금융 지원제도).
