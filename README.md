...
- **에이전트**: LangGraph 라우팅 그래프 — router(의도분류) + 상품군별 worker
- **범위 제한**: 금융과 무관한 질문은 라우팅 단계에서 끊어 LLM을 태우지 않고 정중히 거절
...
- 결정적 코드(도구·DB·API)는 TDD, LLM 동작은 골든셋 eval로 검증합니다.
...
app/
  ...
  db/      SQLAlchemy 모델·저장소 (대화이력, 상품 임베딩)
  batch/   상품 카탈로그 → 벡터 색인 배치 (변경분만 재임베딩)
  api/     FastAPI 라우터 (챗, 이력)
web/       정적 채팅 UI
